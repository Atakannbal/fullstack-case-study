...
export default class Search extends React.Component {
  constructor(props) {
    ...
    this.updateMovies = debounce(this.updateMovies.bind(this), 500);
      ...
      <React.Fragment>
        <form className="flex-center">
          <div className="searchInput">
            <label className="label center-text" htmlFor="query">
              {" "}
              Search for a movie
            </label>
            <input
        ...
        <div className="stateMessages center-fixed">
          {this.isEmpty() && !this.isLoading() && (
            <p> You can search for a movie... </p>
          )}
          {this.isLoading() && !movies.message && <p> Fetching movies... </p>}
          {movies.message && <p> {movies.message} </p>}
        </div>

        <div>{!movies.message && <MoviesGrid movies={movies} />}</div>
      </React.Fragment>
    );
  }
}
